 ...
 * #remove} are used in conjunction. Use {@code get} to retrieve the first
 * element, and then {@code remove} to remove it after successful processing. If
 * the system crashes after {@code get} and during processing, the element will
 ...
 */
public class LargeQueueFile {
  private static final Logger LOGGER = Logger.getLogger(LargeQueueFile.class.getName());
  ...
  /**
   * Expand threshold of file size.
   */
  static final long EXPAND_THRESHOLD = 1L * 1024 * 1024 * 1024;
   ...
   */
  public LargeQueueFile(File file) throws IOException {
   ...
   * For testing.
   */
  LargeQueueFile(RandomAccessFile raf) throws IOException {
    this.raf = raf;
    readHeader();
  }
   ...
   */
  private static void writeLongs(byte[] buffer, long... values) {
    int offset = 0;
   ...
   */
  public final long readLong(byte[] buffer, int offset) {
   ...
   */
  private void writeHeader(long fileLength, long elementCount, long firstPosition, long lastPosition) throws
      IOException {
  ...
  private static void initialize(File file) throws IOException {
    ...
    try {
      ...
      writeLongs(headerBuffer, INITIAL_LENGTH, 0, 0, 0);
  ...
  public synchronized void add(byte[] data, int offset, int count) throws IOException {
    ...
    long position = wasEmpty ? HEADER_LENGTH : wrapPosition(last.position + Element.HEADER_LENGTH + last.length);
  ...
  private void expandIfNecessary(int dataLength) throws IOException {
    ...
    do {
      ...
      if (previousLength < EXPAND_THRESHOLD) {
        newLength = previousLength << 1;
      } else {
        newLength = previousLength + EXPAND_THRESHOLD;
      }
    ...
    if (endOfLastElement <= first.position) {
      ...
      int count = (int) (endOfLastElement - HEADER_LENGTH);
